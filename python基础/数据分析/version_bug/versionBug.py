...
def bar_kpi():
    ...
    release_kpi.plot(kind='bar',stacked=True,rot=30)


def pie_kpi():
    bug_kpi = pd.DataFrame({"Name":["tom","zhangsan","lisi","haven"],"Bug":[0,1,2,3]})
    bug_kpi.set_index("Name",inplace=True)
    bug_kpi.plot(kind='pie',subplot=True,figsize=(8,4))
    bug_kpi.plot(kind='pie', autopic='%.2f', figsize=(8, 4))
    bug_kpi.plot(kind='pie', autopic='%.2f', legend=False,title="Bug Distribution")
    plt.axes().set_ylabel('')
    plt.show()


# duration_kpi = pd.DataFrame({"sessionId":["s1","s2","s3","s4","s5","s6","s7"],"request":[],"response":[]})
